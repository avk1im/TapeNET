...
- **Layered read pipeline** — `WorkerThreadTapeReadBackend` (`ITapeReadBackend`) issues `Drive.ReadDirect` calls on a dedicated worker thread (one block at a time via `ReadOneBlock(byte[] buffer, int offset)`). `TapeFilePipelinedReader` owns a ring buffer of `slotCount × BlockSize` bytes (default 16 slots, rented from `ArrayPool<byte>.Shared`) and a worker thread that prefetches greedily forward from the current tape position, filling slots while the agent consumes already-delivered blocks. `BeginRead(TapeAddress, long)` arms the worker with a per-file `[startBlock, endBlockExcl)` window — the upper bound prevents the prefetcher from consuming the content-setmark that follows the file. Seek-and-resume (in-window address) avoids tape I/O entirely; seek-and-restart (backward / non-monotonic address) flushes the ring and redirects the worker. `TapeReadStreamFacade` is the per-file `Stream` façade, unchanged. The pipelined reader is disposed and recreated on every set boundary.
