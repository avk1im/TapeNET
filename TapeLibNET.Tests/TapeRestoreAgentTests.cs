...
#pragma warning restore CA1825 // Avoid zero-length array allocations
    ...
    public void SingleSet_RestoreToDisk_ByteForByteMatch(DriveProfile profile)
    {
        ...
        try
        {
            ...
            Assert.True(success,
                ...
                $"Failures=[{string.Join("; ", notifiable.FilesFailed.Select(f => $"{f.FileDescr.FullName}: {f.Exception.Message}"))}]");
    ...
    public void Restore_MixedEdgeCaseFiles_Succeeds(DriveProfile profile)
    {
        ...
        try
        {
            ...
            Assert.True(success,
                ...
                $"Failures=[{string.Join("; ", restoreNotifiable.FilesFailed.Select(f => $"{f.FileDescr.FullName}: {f.Exception.Message}"))}]");
    ...
    public void DiagHeaderMismatch_TwoSets_ValidateEachAfterBackup(DriveProfile profile)
    {
        ...
        Assert.True(v1ok,
            ...
            $"Failures=[{string.Join("; ", v1not.FilesFailed.Select(f => $"{f.FileDescr.FullName}: {f.Exception.Message}"))}]");

        // Validate set 2 (no TOC reload)
        var (v2ok, v2not) = ValidateSet(fixture, 2);
        Assert.True(v2ok,
            $"[NO RELOAD] Validate set 2 failed for {profile}: " +
            $"Failures=[{string.Join("; ", v2not.FilesFailed.Select(f => $"{f.FileDescr.FullName}: {f.Exception.Message}"))}]");
    }
