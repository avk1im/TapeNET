...
///  <see cref="TapeFileInfo"/> entries from one or more backup sets.
///  Covers plan items A-3 and A-4.
/// </summary>
public class ServiceSelectiveRestoreTests : ServiceTestBase
{
    /// <summary>
    /// Verifies that a restore honours an explicit <see cref="TapeFileInfo"/> list
    ...
    /// </summary>
    [Fact]
    public async Task SelectiveRestore_SpecificFiles_ByTapeFileInfo()
    {
        ...
        var (svcR, hostR) = await ReopenAsync(media);
        using (svcR)
        {
            var toc = svcR.TOC!;
            ...
            // Pick first PickFromSet1 entries from set 1 and first PickFromSet2 from set 2
    ...
    /// <summary>
    /// Verifies that a single <see cref="RestoreRequest"/> can select specific files
    ///  from two different backup sets simultaneously, and that the combined result
    ///  counts and byte content are both correct.
    /// <para>
    ...
    /// </summary>
    [Fact]
    public async Task SelectiveRestore_AcrossMultipleSets_MixedSelection()
    {
        using var media = new TempVirtualMedia(withInitiator: true, ContentCapacity, InitiatorCapacity);
        using var src1  = new TempFileTree();
        using var src2  = new TempFileTree();
        ...
        using (svcR)
        {
            ...
            // Even-indexed TapeFileInfo entries from set 1; odd-indexed from set 2
            IReadOnlyList<TapeFileInfo> sel1 =
                [.. toc[s1].Where((_, i) => i % 2 == 0)];
            IReadOnlyList<TapeFileInfo> sel2 =
                [.. toc[s2].Where((_, i) => i % 2 == 1)];

            // Matching source-file paths for byte comparison (same index logic)
