...
public partial class TapeServiceBase(ILoggerFactory loggerFactory, ITapeServiceHost host) : IDisposable
{
    ...
    private Grpc.Net.Client.GrpcChannel? _remoteChannel;
            ...
            try
            {
                ...
                if (hasPersistentSession)
                {
                    ...
                    bool swapOk = await backend.InsertMediaAsync(
                            ...
                            protoCaps, mediaMode: System.IO.FileMode.Create)
    ...
    public void Dispose()
    {
        ...
        // Best-effort synchronous cleanup of the persistent remote connection.
        // Atomically take ownership of the channel so a concurrent ClearRemoteConnectionAsync()
        //  call (e.g. from DisconnectRemoteHostAsync) cannot race us between null-check and Dispose.
        var remoteChannel = Interlocked.Exchange(ref _remoteChannel, null);
        if (remoteChannel is not null)
        {
            // Send Close RPC synchronously before disposing the channel.
            var sessionId = _remoteSessionId;
            _remoteSessionId    = null;
            _remoteHostSettings = null;
            ...
            if (!string.IsNullOrEmpty(sessionId))
            {
                try
                {
                    var client = new TapeDriveService.TapeDriveServiceClient(remoteChannel);
                    var opts = new Grpc.Core.CallOptions(
                        headers: new Grpc.Core.Metadata { { "x-tape-session-id", sessionId } });
