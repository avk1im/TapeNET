...
public partial class TapeService
{
                ...
                try
                {
                    ...
                    for (int setIndex = startIndex; setIndex >= endIndex; )
                    {
                        ...
                        ITapeFileFilter? listFilter = options.Filter
                            ?? (patterns.Count > 0 ? new FclTapeFileFilter([.. patterns]) : null);
                        ...
                        for (int i = 0; i < tfisBySets.Length; i++)
                        {
                            ...
                            if (toc.CurrentSetTOC.FmksMode)
                            {
                                ...
                                foreach (var index in indexes)
                                {
                                    ...
                                    _ux.Log(new LogEntry(WarningLevel.None, FormatFileInfoIndex(tfi, index, options.ShowFullPath)));
                                    setFiles++;
                            ...
                            else
                            {
                                ...
                                {
                                    _ux.Log(new LogEntry(WarningLevel.None, FormatFileInfo(tfi, options.ShowFullPath)));
                                    setFiles++;
                                    setSize += tfi.FileDescr.Length;
                                }
                            }
    ...
    #region Private — Listing helpers

    private void LogMediaInfoFull(TapeTOC toc)
    {
    ...
    private void LogCurrentSetInfo(TapeTOC toc)
    {
        if (_drive is null) return;
