...
static ConsoleKey GetConsoleKey(string prompt)
{
    ...
    // output the pressed key if representable
...
void HandleBackup(List<string> values)
{
    ...
    try
    {
        ...
        OnFileBackupProcessor proc = new(toc, quiteMode);
...
abstract class OnFileEventProcessor(TapeTOC toc, bool quiteMode, string processingName = "Processing") : ITapeFileNotifiable
{
    ...
    protected static ConsoleKey GetConsoleKey(string prompt)
    {
        // empty keyboard buffer
        while (Console.KeyAvailable)
            Console.ReadKey(intercept: true);
    ...
    protected FileFailedAction MessageGetFileAction(string msg)
    {
        if (quiteMode)
        {
            Console.WriteLine(msg + " -> Quite mode -> Assuming Skip");
            return FileFailedAction.Skip;
        }

        switch (GetConsoleKey(msg + "\n ?? Press 'R' for Retry, 'A' for Abort, or any other key to Skip: "))
        {
            case ConsoleKey.R:
...
class OnFileValidateProcessor(TapeTOC toc, bool quiteMode) : OnFileEventProcessor(toc, quiteMode, "Validating")
{
} // class OnFileValidateProcessor

class OnFileVerifyProcessor(TapeTOC toc, bool quiteMode) : OnFileEventProcessor(toc, quiteMode, "Verifying")
{
